...
Proof. simpl. reflexivity. Qed.
  ...
  | h1 :: t1 => if v =? h1 then true else member v t1
...
(** When [remove_one] is applied to a bag without the number to
    ...
    | h1 :: t1 => if v =? h1 then t1 else h1 :: (remove_one v t1)
...
Proof. simpl. reflexivity. Qed.
  ...
  | h1 :: t1 => if v =? h1 then remove_all v t1 else h1 :: (remove_all v t1)
...
Proof. simpl. reflexivity. Qed.
  ...
  | h :: t => if (count h s2) <? (count h s1) then false else included t s2
...
Proof.
  ...
  intros l1 l2. induction l1 as [| h t IHt].
...
Proof.
  ...
  - simpl. rewrite rev_app_distr. rewrite -> IHIHl'. simpl. reflexivity.
...
Proof.
  ...
  - simpl. rewrite eqb_refl. rewrite IHIHl'. reflexivity.
...
Proof.
  ...
  induction s as [| s' IHs']; reflexivity.
...
Proof.
  ...
  simpl. induction x, y. rewrite H. reflexivity.
