...
(** Before getting started on this chapter, we need to import
    ...
    *)
    ...
    Proof.
      ...
      - simpl. rewrite H. rewrite <-mult_plus_distr_r. reflexivity.
    ...
    Proof.
      ...
      - simpl. rewrite <- plus_n_Sm. rewrite -> H. reflexivity.
    ...
    induction b as [| a' | c']; reflexivity.
    ...
    Compute nat_to_bin (bin_to_nat  (B1 (B0 Z))).
    
    (* Simplifying the above example, we will get (B1 Z). Although
    
    both the binary numbers are same but their representation is different. 
    
    It is vital that for a theorem to run, the representation should be the same. 
    
    Since that is not the case here, the theorem will not work. *)
    ...
    Fixpoint normalize (b:bin) : bin :=
      ...
      | B0 r => double_bin (normalize r)
    
      | B1 r => incr (double_bin (normalize r))
    ...
    - simpl. rewrite add_assoc. rewrite <- double_plus. destruct b'.
    
      + simpl. reflexivity. 
    
      + simpl. rewrite add_assoc. rewrite <- double_plus. rewrite add_0_r. rewrite add_0_r. rewrite nat_to_bin_double. simpl in IHb'. rewrite -> add_assoc in IHb'. 
    
      rewrite <- double_plus in IHb'. 
    
      rewrite <- nat_to_bin_double. rewrite <- IHb'. rewrite <- nat_to_bin_double. rewrite add_0_r. reflexivity.
    
      + rewrite <- IHb'. rewrite add_0_r. rewrite nat_to_bin_double. reflexivity.
    
    - simpl. destruct (bin_to_nat b'') eqn:E.
    
      + simpl. rewrite <- IHb''. simpl. reflexivity.
    
      + rewrite add_0_r. rewrite <- double_plus. rewrite <- IHb''. rewrite nat_to_bin_double. reflexivity.
